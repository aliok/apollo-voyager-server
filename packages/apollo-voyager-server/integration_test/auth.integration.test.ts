...
process.env.KEYCLOAK_CONFIG_FILE = path.resolve('./integration_test/config/keycloak.json')

import fs from 'fs'
import test from 'ava'
import axios from 'axios'
import * as gqls from './auth.integration.test.gql'
import * as localKeycloak from './auth.integration.test.keycloak'

const context = {
  helper: undefined,
  testNote: 'auth, inmem',
  testPassword: 'admin',
  keycloakConfig: JSON.parse(fs.readFileSync(process.env.KEYCLOAK_CONFIG_FILE) as any)
}
...
async function authenticate (test, username, password) {
  ...
  const authHeaders = await authenticateKeycloak(context.keycloakConfig, username, password)
